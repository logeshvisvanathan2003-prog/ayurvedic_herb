...
export default function ConsumerRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validate inputs
      if (!formData.fullName || !formData.email || !formData.password || !formData.confirmPassword || !formData.phone || !formData.address) {
      ...
      localStorage.setItem('consumerEmail', formData.email);
      localStorage.setItem('consumerName', formData.fullName);
      localStorage.setItem('consumerPhone', formData.phone);
      localStorage.setItem('consumerAddress', formData.address);

      setSuccess(true);

      // Auto-login after registration
      login(formData.email, 'consumer');

      // Redirect to consumer portal after 2 seconds
      setTimeout(() => {
