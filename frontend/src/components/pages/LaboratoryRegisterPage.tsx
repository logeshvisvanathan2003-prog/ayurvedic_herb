...
export default function LaboratoryRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validate inputs
      if (!formData.labName || !formData.email || !formData.password || !formData.confirmPassword || !formData.labCode || !formData.contactPerson || !formData.phone) {
      ...
      localStorage.setItem('labEmail', formData.email);
      localStorage.setItem('labName', formData.labName);
      localStorage.setItem('labCode', formData.labCode);
      localStorage.setItem('contactPerson', formData.contactPerson);
      localStorage.setItem('labPhone', formData.phone);

      setSuccess(true);

      // Auto-login after registration
      login(formData.email, 'lab');

      // Redirect to laboratory testing portal after 2 seconds
      setTimeout(() => {
            ...
            <Card className="p-8 border-2 border-primary bg-white shadow-lg text-center">
              <div className="mb-6">
                ...
                <p className="font-paragraph text-secondary">
                  Welcome, {formData.labName}! Redirecting to your dashboard...
                </p>
              </div>
            </Card>
          </div>
